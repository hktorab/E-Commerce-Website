	...
	<table>
		...
		while ($row=$res->fetch_object()) {
			if (!(empty($row->deliveredBy))) {
				...
				$delMan=$conn->query("select * from user where userId=".$row->deliveredBy.";");
				$delManName=$delMan->fetch_object();
				...
				for ($i=0; $i < $arr_length; $i++) { 
					$sql_query="select * from product where product_id=".$arr[$i];
					$resShow=$conn->query($sql_query);
					...
					<tr>
						...
						<td><?php echo $delManName->Fname." ".$delManName->Lname ;?></td>
